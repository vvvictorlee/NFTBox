...
export class ScanAPI {
        ...
        while (true) {
            try {
                const res = await fetch(
                    "https://api.hooscan.com//api?module=account&action=txlist&address=" + address + "&startblock=" + blocknumber + "&endblock=99999999&page="+page+"&offset=10000&sort=asc&apikey=YourApiKeyToken"
                );
                const headerDate =
    ...
    async testtxtotal(address) {
        await apidbmgmt.init();
        apidbmgmt.getTxGasedTotalByAccount(address);
    }

    async testtxcontract(address) {
        await apidbmgmt.init();
        apidbmgmt.getTxGasedTotalByAccountAndContract(address);
    }
    async testel(address) {
        await apidbmgmt.init();
        apidbmgmt.getEarliestAndLatestTxByAccount(address);
    }
}
